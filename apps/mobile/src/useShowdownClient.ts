...
export function useShowdownClient(credentials?: Credentials): ShowdownClientHandle {
  ...
  useEffect(() => {
    const unsubscribers = [
      ...
      client.on('message', ({ roomid, args }) => {
        // Chat lines only, for now — battle protocol lines get their own
        // renderer once the battle UI exists. @pkmn/protocol normalises the
        // wire shorthand (`|c|`) to `"chat"`; `"c:"` is the distinct
        // timestamped form used for room history on join and has no long form.
        if (args[0] === 'chat') {
  ...
  return { connectionState, session, username, authError, log, join, say };
}
